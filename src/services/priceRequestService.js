...
import { getProductById, resolveProductId, updateProduct } from './productService.js';
...
export async function submitPriceChangeRequest({
  ...
  // Legacy catalog-snapshot entries can be missing `id`; recover the real
  // Firestore document id via sr instead of rejecting the request outright.
  const productId = product?.id || (await resolveProductId(product));
